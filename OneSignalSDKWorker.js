 ...
 *   - stale-while-revalidate for the app shell (index.html) -> instant open, refreshes in the background
...
var SHELL_CACHE = "wc2026-shell-v1";
var SHELL = [
...
self.addEventListener("fetch", function (e) {
  ...
  if (req.mode === "navigate" || p === "/" || p.slice(-1) === "/" || p.indexOf("index.html") > -1) {
    e.respondWith(caches.open(SHELL_CACHE).then(function (c) {
      var net = fetch(req).then(function (r) { if (r && r.ok) c.put("index.html", r.clone()); return r; }).catch(function () { return null; });
      return c.match("index.html").then(function (cached) { return cached || net.then(function (r) { return r || fetch(req); }); });
    }));
    return;
  }
